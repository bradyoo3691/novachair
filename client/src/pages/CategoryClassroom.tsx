...
const CLASSROOM_IDS = ['nc-002', 'nc-003', 'nc-004', 'nc-005', 'nc-010'];

export default function CategoryClassroom() {
  const products = PRODUCTS.filter((p) => CLASSROOM_IDS.includes(p.id));

  return (
    <main className="min-h-screen bg-white">
      <section className="bg-[#1C1C1E] py-14">
        <div className="container">
          <p className="text-[#C4714A] text-xs tracking-[0.2em] uppercase mb-3">CLASSROOM CHAIR</p>
          <h1 className="nova-heading text-4xl md:text-5xl text-[#F5F0EB] mb-4">학원 / 강의실 의자</h1>
          <p className="text-[#888] text-base max-w-xl leading-relaxed">
            학원, 강의실, 교육 시설에 최적화된 의자 라인업입니다.
          </p>
        </div>
      </section>
