...
const RESTAURANT_IDS = ['nc-001'];

export default function CategoryRestaurant() {
  const products = PRODUCTS.filter((p) => RESTAURANT_IDS.includes(p.id));

  return (
    <main className="min-h-screen bg-white">
      <section className="bg-[#1C1C1E] py-14">
        <div className="container">
          <p className="text-[#C4714A] text-xs tracking-[0.2em] uppercase mb-3">RESTAURANT CHAIR</p>
          <h1 className="nova-heading text-4xl md:text-5xl text-[#F5F0EB] mb-4">고깃집 의자</h1>
          <p className="text-[#888] text-base max-w-xl leading-relaxed">
            고깃집, 식당에 최적화된 깡통의자 라인업입니다. 다양한 색상으로 공간을 연출하세요.
          </p>
        </div>
      </section>
